...
@section('content')
    ...
    <div class="card w-50 mx-auto mt-5">
        <div class="card-body">
            ...
            <h5 class="modal-title text-primary">Send Email to reschedule Date</h5>
            <hr />
            <form action="{{ route('consultation.emailstore') }}" method="post" enctype="multipart/form-data">
                @csrf
                <div class="mb-3">
                    <label for="title" class="form-label">Email</label>
                    <input type="text" name="reciever" class="form-control @error('reciever') is-invalid  @enderror"
                        id="reciever" value="{{ $email }}">
                    @error('reciever')
