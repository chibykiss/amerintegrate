...
@section('content')
    <div class="card w-50 mx-auto mt-5">
        <div class="card-body">
            ...
            <h5 class="modal-title text-primary">Send Email to All subscribers</h5>
            <hr />
            <form action="{{ route('email.sendbulk') }}" method="post" enctype="multipart/form-data">
                @csrf
                <div class="mb-3">
                    <label for="title" class="form-label">Subject</label>
                    <input type="text" name="subject" class="form-control @error('subject') is-invalid  @enderror"
                        id="title" value="{{old('subject')}}">
                    @error('subject')
