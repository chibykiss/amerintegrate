...
@section('content')
    ...
    <div class="card w-50 mx-auto mt-5">
        <div class="card-body">
            ...
            <h5 class="modal-title text-primary">Resend Email</h5>
            <hr />
            <form action="{{ route('email.resend') }}" method="post" enctype="multipart/form-data">
                @csrf
                <div class="mb-3">
                    <label for="title" class="form-label">Email</label>
                    <input type="text" name="reciever" class="form-control @error('reciever') is-invalid  @enderror"
                        id="reciever" value="{{ $newsletter->via }}">
                    @error('reciever')
                ...
                <div class="mb-3">
                    ...
                    <input type="text" name="subject" class="form-control @error('subject') is-invalid  @enderror"
                        id="subject" value="{{ $newsletter->subject }}">
                    @error('subject')
                        <span class="invalid-feedback" role="alert">
                            <strong>{{ $message }}</strong>
                        </span>
                ...
                <div class="mb-3">
                    ...
                    <textarea class="form-control @error('emailbody') is-invalid  @enderror" name="emailbody" id="editor" rows="4"
                        cols="50">{{ $newsletter->body }}</textarea>
                    @error('emailbody')
                        <span class="invalid-feedback" role="alert">
                            <strong>{{ $message }}</strong>
                        </span>
